def experiment_prep_and_run(subbb, seed):
    ...
    # ---------- subjects ----------
    subs_considered = subbb


    # ---------- params ----------
    nr_of_epochs = 300
    nb_classes   = 2
    val_size     = 0.2 # .25 for 1.4 ratio for B case, ow it's .2
    test_size    = 0.2 # .25 for 1.4 ratio for B case, ow it's .2
    num_trials   = 24
    per_sub =True  # zero training flag
    add_rest = True # add ref flag

    # ---------- data ----------
    X, y = load_data(subs_considered, sample_rate=250, bp_low=6, bp_high=40, notch_f=50,normalize=True, tmin_MI = 0, tmax_MI = 3)
    ...
    y = to_categorical(y-1, nb_classes)
